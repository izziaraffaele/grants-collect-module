...
library TestHelpers {
  ...
  bytes32 constant profileCreatedEventTopic =
    keccak256("ProfileCreated(uint256,address,address,string,string,address,bytes,string,uint256)");
  ...
  function getTransferFromEvents(Vm.Log[] memory entries, address from, address to) public pure returns (uint256) {
    for (uint256 i = 0; i < entries.length; i++) {
      if (entries[i].topics[0] == transferEventTopic) {
        if (entries[i].topics[1] == bytes32(abi.encode(from)) && entries[i].topics[2] == bytes32(abi.encode(to))) {
